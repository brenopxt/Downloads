...
def receber_mensagem():
    try:
        ...
        # 🛡️ PROTEÇÃO: ignora mensagens antigas (sincronização de histórico ao reconectar)
        timestamp_msg = mensagem.get("timestamp", 0)
        agora = datetime.now().timestamp()
        if agora - timestamp_msg > 120:  # mais de 2 minutos de idade
            log(f"⏳ Mensagem antiga ignorada (idade: {int(agora - timestamp_msg)}s)")
            return jsonify({"status": "mensagem_antiga_ignorada"})
